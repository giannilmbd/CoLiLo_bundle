...
!Changed by: giovanni lombardo (dropped allocate from spline_interp)
! changed also d to e and used my kinds. Thus changed thresholds from 1d-100 to 1e-7
...
! Level of tolerance for all routines
real(wp),  private  :: tbox_gftol_root = 1e-6
...
real(wp), allocatable, private :: gnu_x(:, :), gnu_y(:, :)
real(wp), allocatable, private :: gnu_x_temp(:, :), gnu_y_temp(:, :)
...
public :: discretize_AR, discretize_log_AR, discretize_AR_NG
...
! Added for faster computations
public :: spline3_grid_2d, spline3_2d,spline2_grid_2d, spline2_2d
...
interface fzero
    ...
    module procedure newton_interpol
    module procedure broydn
...
!##############################################################################
    ...
    use my_kinds_mod
        ...
        ! get number of days
        times(1) = floor(time/(24e0*60e0*60e0))
        time = time - times(1)*24e0*60e0*60e0
        
        ! get number of hours
        times(2) = floor(time/(60e0*60e0))
        time = time - times(2)*60e0*60e0
        
        ! get number of minutes
        times(3) = floor(time/60e0)
        time = time - times(3)*60e0
...
!##############################################################################
    ...
    subroutine grid_Cons_Cheb(a, left, right)
        ...
        integer :: j, n
        real(wp), parameter :: pi = 3.1415926535897932e0
        ...
        ! calculate grid value
        a = (left+right)/2e0 + (right-left)/2e0* &
            cos((real(n)-(/(real(j), j=0,n)/)+0.5e0)/real(n+1)*pi)
    ...
    subroutine grid_Cons_Grow(a, left, right, growth)
        ...
        ! check for growth
        if(growth <= 0e0)call error('grid_Cons_Grow', &
            'growth rate must be greater than zero')

        ! calculate factor
        h = (right-left)/((1e0+growth)**n-1e0)
     
        ! calculate grid value
        a = h*((1e0+growth)**(/(real(j), j=0,n)/)-1e0)+left
    ...
    function grid_Val_Cheb(x, left, right, n)
        ...
        ! calculate grid value
        grid_Val_Cheb = (left+right)/2e0 + (right-left)/2e0* &
            cos((real(n)-x+0.5e0)/real(n+1)*pi)
    ...
    function grid_Val_Grow(x, left, right, growth, n)
        ...
        ! check for growth
        if(growth <= 0e0)call error('grid_Val_Grow', &
            'growth rate must be greater than zero')
    ...
    function grid_Inv_Cheb_1(x, left, right, n)
        ...
        ! calculate grid value
        grid_Inv_Cheb_1 = real(n) + 0.5e0 - acos((2e0*x-(left+right)) &
            /(right-left))*real(n+1)/pi
    ...
    function grid_Inv_Grow_1(x, left, right, growth, n)
        ...
        ! check for growth
        if(growth <= 0e0)call error('grid_Inv_Grow', &
            'growth rate must be greater than zero')
     
        ! calculate factor
        h = (right-left)/((1+growth)**n-1e0)
     
        ! calculate grid value
        grid_Inv_Grow_1 = log((x-left)/h+1e0)/log(1e0+growth)
    ...
    function grid_Inv_Equi_m(x, left, right, n)
        ...
        ! value of the inverse of the gridpoint x \in [left, right]
        real(wp) :: grid_Inv_Equi_m(size(x, 1))
    ...
    function grid_Inv_Cheb_m(x, left, right, n)
        ...
        ! value of the inverse of the gridpoint x \in [left, right]
        real(wp) :: grid_Inv_Cheb_m(size(x, 1))
        ...
        ! calculate grid value
        grid_Inv_Cheb_m = real(n) + 0.5e0 - acos((2e0*x-(left+right)) &
            /(right-left))*real(n+1)/pi
    ...
    function grid_Inv_Grow_m(x, left, right, growth, n)
        ...
        ! value of the inverse of the gridpoint x \in [left, right]
        real(wp) :: grid_Inv_Grow_m(size(x, 1))
        ...
        ! calculate grid value
        grid_Inv_Grow_m = log((x-left)/h+1e0)/log(1e0+growth)
    ...
    subroutine linint_Equi_m(x, left, right, n, il, ir, phi)
        ...
        integer :: m
        real(wp) :: h, xinv(size(x, 1)), xl(size(x, 1)), xr(size(x, 1))
    ...
    subroutine linint_Cheb_1(x, left, right, n, il, ir, phi)
        ...
        ! determine left and right gridpoint        
        xl = (left+right)/2e0 + (right-left)/2e0* &
            cos((real(n)-real(il)+0.5e0)/real(n+1)*pi)
        xr = (left+right)/2e0 + (right-left)/2e0* &
            cos((real(n)-real(ir)+0.5e0)/real(n+1)*pi)
    ...
    subroutine linint_Cheb_m(x, left, right, n, il, ir, phi)
        ...
        integer :: m
        real(wp) :: xinv(size(x, 1)), xl(size(x, 1)), xr(size(x, 1))
        real(wp), parameter :: pi = 3.1415926535897932e0
        ...
        ! invert the grid to get point
        xinv = grid_Inv_Cheb_m(min(max(x, left), right), -1e0, 1e0, n)
    ...
    subroutine linint_Grow_1(x, left, right, growth, n, il, ir, phi)
        ...
        ! determine left and right gridpoint
        h = (right-left)/((1+growth)**n-1)     
        xl = h*((1+growth)**real(il)-1e0)+left
        xr = h*((1+growth)**real(ir)-1e0)+left
    ...
    function linint_Gen(x, xi, yi, istart_in)
        ...
        phi = 1e0 - (x-xi(ial))/(xi(iar)-xi(ial))        
        linint_Gen = phi*yi(ial) + (1e0-phi)*yi(iar)
...
!##############################################################################
    ...
    subroutine lu_solve(a, b)
        ...
        integer :: indx(size(b))
        real(wp) :: worka(size(a, 1), size(a, 2))
        real(wp) :: d
        integer :: n
    ...
    function lu_invert(a)
        ...
        real(wp) :: lu_invert(size(a, 1), size(a, 2))
    ...
    subroutine lu_decomp(a, indx, d)
        ...
        real(wp) :: vv(size(a,1))
        real(wp), parameter :: tiny = 1.0e-20
        integer :: j, n, imax
        ...
        if (any(abs(vv) <= 1e-7)) call error('lu_decomp', 'matrix is singular')
        ...
        do j = 1, n
            ...
            if(abs(a(j,j))  <= 1e-7)call error('lu_decomp', 'matrix is singular')
    ...
    contains
        ...
        function outerprod(a, b)
            ...
            real(wp) :: outerprod(size(a, 1),size(b, 1))
    ...
    subroutine lu_dec(a, l, u)
        ...
        real(wp) :: Awork(size(a,1), size(a,2))
    ...
    subroutine cholesky(a, l)
        ...
        do i = 1, n
            ...
            if(summ <= 0e0)call error('cholesky', &
...
!##############################################################################
    ...
    function normalPDF(x, mu, sigma)
        ...
        real(wp) :: mu_c, sigma_c
        real(wp), parameter :: pi = 3.1415926535897e0
        ...
        normalPDF = 1e0/(sigma_c*sqrt(2e0*pi))*exp(-((x-mu_c)/sigma_c)**2/2e0)
    ...
    function normalCDF(x, mu, sigma)
        ...
        real(wp) :: mu_c, sigma_c, xtrans, xabs, y
        real(wp), parameter :: a0  = 0.5e0
        real(wp), parameter :: a1  = 0.398942280444e0
        real(wp), parameter :: a2  = 0.399903438504e0
        real(wp), parameter :: a3  = 5.75885480458e0
        real(wp), parameter :: a4  = 29.8213557808e0
        real(wp), parameter :: a5  = 2.62433121679e0
        real(wp), parameter :: a6  = 48.6959930692e0
        real(wp), parameter :: a7  = 5.92885724438e0
        real(wp), parameter :: b0  = 0.398942280385e0
        real(wp), parameter :: b1  = 3.8052e-8
        real(wp), parameter :: b2  = 1.00000615302e0
        real(wp), parameter :: b3  = 3.98064794e-4
        real(wp), parameter :: b4  = 1.98615381364e0
        real(wp), parameter :: b5  = 0.151679116635e0
        real(wp), parameter :: b6  = 5.29330324926e0
        real(wp), parameter :: b7  = 4.8385912808e0
        real(wp), parameter :: b8  = 15.1508972451e0
        real(wp), parameter :: b9  = 0.742380924027e0
        real(wp), parameter :: b10 = 30.789933034e0
        real(wp), parameter :: b11 = 3.99019417011e0
        ...
        if(present(sigma))then
            if(sigma_c > 0e0)sigma_c = sqrt(sigma)
        endif
        ...
        if(xtrans > 0e0)normalCDF = 1e0-normalCDF
    ...
    function normalCDF_Inv(x, mu, sigma)
        ...
        real(wp) :: normalCDF_Inv
     
     
        !##### OTHER VARIABLES ####################################################
     
        real(wp) :: mu_c, sigma_c, q, r
        real(wp), parameter :: a(8) = (/  3.3871328727963666080e0, &
        ...
        real(wp), parameter :: c(8) = (/  1.42343711074968357734e0, &
        ...
        real(wp), parameter :: const1 = 0.180625e0
        real(wp), parameter :: const2 = 1.6e0
        real(wp), parameter :: d(8) = (/  1.0e0, &
        ...
        real(wp), parameter :: e(8) = (/  6.65790464350110377720e0, &
        ...
        real(wp), parameter :: split1 = 0.425D+00
        real(wp), parameter :: split2 = 5.0D+00
    ...
    function log_normalPDF(x, mu, sigma)
        ...
        sigma_c = log(1e0+sigma_c/mu_c**2)
        mu_c  = log(mu_c)-0.5e0*sigma_c
    ...
    function GammaPDF(x, alpha, beta)
        ...
        if(x < 0e0 .or. alpha_c < 1e0 .and. x <= 0e0)then
            GammaPDF = 0e0
        else
            GammaPDF = beta_c**alpha_c/gamma_function(alpha_c)* &
                        x**(alpha_c-1e0)*exp(-beta_c*x)
        endif
    ...
    function betaCDF(x, p_in, q_in)
        ...
        ! parameter of the distribution
        real(wp), optional :: p_in
     
        ! parameter of the distribution
        real(wp), optional :: q_in
        ...
        real(wp), parameter :: acu = 0.1e-14
        real(wp) :: p, q, ai, beta_log, cx, pp, psq, qq, rx, temp, term, xx
        integer :: ns
        logical :: indx
    ...
    function incomplete_gamma(x, p)
        ...
        ! value of normal density at p
        real(wp) :: incomplete_gamma
     
     
        !##### OTHER VARIABLES ####################################################
     
        real(wp) :: a, an, arg, b, c, pn1, pn2, pn3, pn4, pn5, pn6, rn
        real(wp), parameter :: elimit = - 88e0
        real(wp), parameter :: oflo = 1e37
        real(wp), parameter :: plimit = 1000e0
        real(wp), parameter :: tol = 1e-14
        real(wp), parameter :: xbig = 1e8
     
     
        !##### ROUTINE CODE #######################################################

        incomplete_gamma = 0e0
        ...
        if(p > plimit)then
            pn1 = 3e0*sqrt(p)*((x/p)**(1e0/3e0) + 1e0/(9e0*p) - 1e0)
        ...
        if(x > xbig)then
            incomplete_gamma = 1e0
        endif
        
        ! Pearson series expansion
        if(x <= 1e0 .or. x < p)then
            arg = p*log(x) - x - my_log_gamma(p+1e0)
            c = 1e0
    ...
    function my_log_gamma(x_in)
        ...
        ! value of log of the gamma function
        real(wp) :: my_log_gamma
     
     
        !##### OTHER VARIABLES ####################################################
     
        real(wp), parameter :: alr2pi = 9.18938533204673e-1
        real(wp), parameter :: xlge = 5.10e6 
        real(wp), parameter :: r1(9) = (/ -2.66685511495e0, &
                                        -2.44387534237e1, &
                                        -2.19698958928e1, & 
                                         1.11667541262e1, &
                                         3.13060547623e0, &
                                         ...
                                         3.14690115749e1, &
                                         1.52346874070e1 /)
        real(wp), parameter :: r2(9) = (/ -7.83359299449e1, &
                                        -1.42046296688e2, &
                                         1.37519416416e2, & 
                                         7.86994924154e1, &
                                         4.16438922228e0, &
                                         ...
                                         2.63505074721e2, &
                                         4.33400022514e1 /)
        real(wp), parameter :: r3(9) = (/ -2.12159572323e5, & 
                                         2.30661510616e5, &
                                         2.74647644705e4, &
                                        -4.02621119975e4, &
                                        -2.29660729780e3, &
                                        ...
                                        -2.42357409629e4, &
                                        -5.70691009324e2 /)
        real(wp), parameter :: r4(5) = (/  2.79195317918525e-1, &
                                         4.917317610505968e-1, &
                                         6.92910599291889e-2, &
                                         3.350343815022304e0, &
                                         6.012459259764103e0 /)
        real(wp) :: x, x1, x2, y
        ...
        elseif(x < 12e0)then
            ...
            y = log(x)
            my_log_gamma = x * (y -1e0) - 0.5e0*y + alr2pi
            if(x <= XLGE)then
                x1 = 1e0/x
                x2 = x1 * x1
    ...
    function bernoulliPDF(k, p)
        ...
        real(wp) :: bernoulliPDF
        
             
        !##### ROUTINE CODE #######################################################
        
        ! check for validity
        if(p < 0e0 .or. p > 1e0)then
    ...
    function binomialPDF(k, n, p)
        ...
        binomialPDF = binomial_coefficient(n, k)*p**k*(1e0-p)**(n-k)        
    ...
    function binomialCDF(k, n, p)
        ...
        binomialCDF = 0e0
        do ii = 0, k
            binomialCDF = binomialCDF +  binomial_coefficient(n, ii)*p**ii*(1e0-p)**(n-ii)
        enddo
    ...
    function binomial_coefficient(n, k)
        ...
        binomial_coefficient = 1e0
        do ii = 1, k
            binomial_coefficient = binomial_coefficient*real(n+1-ii)
            binomial_coefficient = binomial_coefficient/real(ii)
        enddo
    ...
    subroutine simulate_normal_1(x, mu, sigma, fixed)
        ...
        real(wp) :: uni1, uni2, mu_c, sigma_c
        real(wp) :: pi = 3.141592653589793e0
        ...
        x = sqrt(-2e0*log(uni1))*cos(2e0*pi*uni2)
    ...
    subroutine simulate_normal_n(x, mu, sigma, fixed)
        ...
        real(wp) :: uni1(size(x, 1)/2), uni2(size(x, 1)/2), mu_c, sigma_c
        integer :: n, in, n2
        real(wp) :: pi = 3.141592653589793e0
        ...
        ! transform by Box-Muller transformation
        do in = 1, n2
            x(2*(in-1)+1) = sqrt(-2e0*log(uni1(in)))*cos(2e0*pi*uni2(in))
            x(2*(in-1)+2) = sqrt(-2e0*log(uni1(in)))*sin(2e0*pi*uni2(in))
        enddo
     
        if(2*n2 /= n)then
            call simulate_normal_1(x(n), 0e0, 1e0)
        endif
    ...
    subroutine simulate_Gamma_1(x, alpha, beta, fixed)
        ...
        real(wp) :: alpha_c, beta_c, d, c, uni
    ...
    subroutine simulate_Gamma_n(x, alpha, beta, fixed)
        ...
        real(wp) :: alpha_c, beta_c, d, c, uni(size(x, 1))
        integer :: n, in
    ...
    function simulate_Gamma_plain(c, d, fixed) result(res)
        ...
        do
            ...
            ! check squeeze
            if(u < 1e0 - 0.0331e0*(x*x)*(x*x))then
                res = d*v
                return
            endif
     
            ! check real condition
            if(log(u) < 0.5e0*x*x + d*(1e0 - v + log(v)))then
                res = d*v
                return
            endif
        enddo
    ...
    subroutine simulate_beta_1(x, p, q, fixed)
        ...
        real(wp) :: p_c, q_c, gamma1, gamma2, bern
        ...
        if(gamma1 > 0e0 .or. gamma2 > 0e0)then
    ...
    subroutine simulate_beta_n(x, p, q, fixed)
        ...
        real(wp) :: p_c, q_c, gamma1(size(x,1)), gamma2(size(x,1)), bern
        integer :: n, in
        ...
        do in = 1, n
            ...
            ! check whether gammas both greaters 0
            if(gamma1(in) > 0e0 .or. gamma2(in) > 0e0)then
    ...
    subroutine normal_discrete_1(x, prob, mu, sigma)
        ...
        real(wp) :: mu_c, sigma_c, pim4, z=0e0, z1, p1, p2, p3, pp
        integer :: n, m, i, j, its
        ...
        do i = 1, m
            ...
            if(i == 1)then
                z = sqrt(real(2*n+1))-1.85575e0*(real(2*n+1)**(-1e0/6e0))
            elseif(i == 2)then
                z = z - 1.14e0*(real(n)**0.426e0)/z
            elseif(i == 3)then
                z = 1.86e0*z+0.86e0*x(1)
            elseif(i == 4)then
                z = 1.91e0*z+0.91e0*x(2);
            else
                z = 2e0*z+x(i-2);
            endif
            ...
            do while(its < maxit)
                ...
                do j = 1, n
                    p3 = p2
                    p2 = p1
                    p1 = z*sqrt(2e0/real(j))*p2-sqrt(real(j-1)/real(j))*p3
                enddo
                pp = sqrt(2e0*real(n))*p2
                z1 = z
                z  = z1-p1/pp
    ...
    subroutine normal_discrete_2(n, x, prob, mu, sigma, rho)
        ...
        ! expectation of distribution
        real(wp), optional :: mu(2)
     
        ! variance of distribution
        real(wp), optional :: sigma(2)
     
        ! correlation of distribution
        real(wp), optional :: rho
     
     
        !##### OTHER VARIABLES ####################################################
     
        real(wp) :: mu_c(2), sig_c(2), rho_c, sigma_c(2,2), l(2,2)
        real(wp) :: x1(n(1)), x2(n(2)), p1(n(1)), p2(n(2))
        integer :: m, j, k
        ...
        if(any(abs(transpose(sigma_c) - sigma_c) > 1e-20)) &
            call error('normal_discrete', &
            'Variance-Covariance matrix is not symmetric')
     
        ! get standard normal distributed random variables
        call normal_discrete(x1, p1, 0e0, 1e0)
        call normal_discrete(x2, p2, 0e0, 1e0)
        ...
        if(.not.any(abs(sig_c)  <= 1e-7))then
    ...
    subroutine log_normal_discrete_2(n, x, prob, mu, sigma, rho)
        ...
        mu_c = exp(0.5e0)
        if(present(mu))mu_c = mu
        sig_c = exp(1e0)*(exp(1e0)-1e0)
        if(present(sigma))sig_c = sigma
        rho_c = 0e0
        if(present(rho))rho_c = rho
        
        if(any(sig_c < 0e0))then
            call error('log_normal_discrete','sigma has negative value')
        endif
        if(any(mu_c <= 0e0))then
            call error('log_normal_discrete','mu has zero or negative value')
        endif
        if(rho_c < -1e0 .or. rho_c > 1e0)then
            call error('log_normal_discrete','rho is outside -1 to 1')
        endif
        
        ! get expectation and variance
        sig_c = log(1e0+sig_c/mu_c**2)
        mu_c  = log(mu_c)-0.5e0*sig_c
        
        ! set up covariance matrix
        sigma_c(1, 1) = sig_c(1)
        sigma_c(2, 2) = sig_c(2)
        sigma_c(1, 2) = log(rho_c*sqrt(exp(sig_c(1))-1e0)* &
            sqrt(exp(sig_c(2))-1e0)+1e0)
        sigma_c(2, 1) = sigma_c(1, 2)
...
!##############################################################################
    ...
    function poly_interpol_1(x, xi, yi)
        ...
        ! return value
        real(wp) :: poly_interpol_1
     
     
        !##### OTHER VARIABLES ####################################################
     
        integer :: j, n, i
        real(wp) :: lagrange_poly(0:size(xi, 1)-1)
    ...
    function poly_interpol_m(x, xi, yi)
        ...
        ! return value
        real(wp) :: poly_interpol_m(size(x, 1))
     
     
        !##### OTHER VARIABLES ####################################################
     
        integer :: j, n, i
        real(wp) :: lagrange_poly(size(x, 1), 0:size(xi, 1))
...
!##############################################################################
    ...
    subroutine brent(xmin, fret, minimum, maximum, func)
        ...
        real(wp), intent(in) :: minimum, maximum
        
        
        !##### OTHER VARIABLES ####################################################
        
        real(wp) :: tol
        real(wp), parameter :: cgold = 0.3819660e0
        real(wp), parameter :: zeps = 1.0e-3*epsilon(xmin)
        real(wp) :: a=0e0, b=0e0, d=0e0, e=0e0, etemp, fu, fv, fw, fx, p, q, r, tol1, tol2, &
            u, v, w, x, xm, ax, bx, cx
        integer :: iter
        ...
        do iter = 1,tbox_itermax_min
            ...
            if(abs(x-xm) <= (tol2-0.5e0*(b-a)))then
                xmin = x
                fret = fx
                return
            endif
            ...
            if(fu <= fx)then
                ...
                if(fu <= fw .or. abs(w-x)  <= 1e-7)then
                    v = w
                    fv = fw
                    w = u
                    fw = fu
                elseif(fu <= fv .or. abs(v-x) <= 1e-7 .or. abs(v-w) <= 1e-7)then
                    v = u
                    fv = fu
                endif
            endif
    ...
    subroutine powell(p, fret, minimum, maximum, func)
        ...
        real(wp), intent(in) :: minimum(:)
    
        ! maximum optimization interval point
        real(wp), intent(in) :: maximum(:)
    
    
        !##### OTHER VARIABLES ####################################################
    
        real(wp) :: xi(size(p, 1), size(p, 1))
        real(wp) :: ftol
        real(wp), parameter :: tiny = 1.0e-25
        integer :: i, ibig, n, iter
        real(wp) :: del, fp, fptt, t
        real(wp), dimension(size(p)) :: pt, ptt, xit
        real(wp) :: xicom(size(p,1))
        ...
        do
            ...
            if (2e0*(fp - fret) <= ftol*(abs(fp) + abs(fret)) + tiny) return
            ...
            ! calculate t
            t = 2e0*(fp - 2e0*fret + fptt) * (fp - fret - del)**2 &
                - del*(fp - fptt)**2
    ...
    contains
        ...
        !##########################################################################
        real(wp) function extr(dim, maxxx, richt)
            ...
            if(richt > 0e0 .and. maxxx == 1 .or. richt < 0e0 .and. maxxx == 0)then
                extr = maximum(dim)
            else
                extr = minimum(dim)
            endif
        ...
        function brent_pow(ax, bx, cx, tol, xmin, func)
            ...
            real(wp), parameter :: cgold = 0.3819660e0
            real(wp), parameter :: zeps=1.0e-3*epsilon(ax)
            integer :: iter
            real(wp) :: a=0e0, b=0e0, d=0e0, e=0e0, etemp=0e0
            real(wp) :: fu, fv, fw, fx, p, q, r, tol1, tol2, &
                u, v, w, x, xm
...
!##############################################################################
    ...
    subroutine solve_lin(x, c, A, b, numle, numge, numeq)
        ...
        integer :: m, n, i1
        real(wp) :: A_h(size(A, 1), size(A, 2)+numle+numge)
        real(wp) :: c_h(size(c, 1)+numle+numge), b_h(size(b, 1))
        real(wp) :: x_h(size(x, 1)+numle+numge)
        real(wp) :: newA(size(A, 1), size(A, 2)+numle+numge)
    ...
    contains
        ...
        subroutine get_starting_value(x0, A, b, newA)
            ...
            real(wp), intent(out) :: newA(:, :)
     
     
            !##### OTHER VARIABLES ####################################################
     
            integer :: m, n, j
            real(wp) :: Astart(size(A,1), size(A,1)+size(A,2))
            real(wp) :: cstart(size(A,1)+size(A,2)), xstart(size(A,1)+size(A,2))
        ...
        subroutine solve_simplex(x, c, A, newA)
            ...
            real(wp), intent(out), optional :: newA(:, :)
            ...
            real(wp) :: alpha(size(A, 1), size(A,2)-size(A, 1))
            real(wp) :: gamma(size(A, 2)-size(A,1)), x0(size(A, 1))
            real(wp) :: pivcheck(size(A,1)), phelp, alpha_h(size(alpha, 2))
            ...
            do
                ...
                do ibas = 1, m
                    if(abs(alpha(ibas, piv(2))) >= 1e-7)then
                ...
                phelp = -1e38
                do ibas = 1, m
                    if(alpha(ibas, piv(2)) < 0e0 .and. pivcheck(ibas) > phelp)then
                        phelp = pivcheck(ibas)
                        piv(1) = ibas
                    endif
                enddo
                ...
                alpha(piv(1), piv(2)) = 1e0/alpha(piv(1), piv(2))
...
!##############################################################################
    ...
    !##############################################################################
    subroutine newton_interpol(x, funcv, check_return,parin_)
        ...
        real(wp), intent(inout) :: x
        real(wp), intent(in),optional :: parin_(:)
        ...
        real(wp) :: tolf, tolmin
        real(wp), parameter :: tolx = eps
        real(wp), parameter :: stpmx = 100e0
        real(wp) :: x1, x2, f1, f2, xnew, fnew, h
        integer :: its
        ...
        if(abs(f1) < 0.01e0*tolf)then
            x = x1
            if(present(check_return))check_return = .false.
            return
        endif
     
        if(abs(f2) < 0.01e0*tolf)then
            x = x2
            if(present(check_return))check_return = .false.
            return
        endif
        
        if(abs((1e0-f1/f2)/(x2-x1)) < tolmin .or. &
            abs((1e0-f1/f2)) < epsilon(1e0))then
            x = x1
        ...
        do its = 1, itermax_root
            ...
            xnew = x2 - (x2-x1)/(1e0-f1/f2)
     
            ! calculate new function value
            fnew = funcv(xnew,parin_)
            ...
            if(2e0*abs(xnew-x2) < tolx*abs(xnew+x2))then
                x = x2
                if(present(check_return))check_return = .true.
                return
            endif
    ...
    !     Computing, 2nd edition. Cambridge: Cambridge University Press. 
    !##############################################################################
    subroutine broydn(x, funcv, check_return,parin)
        ...
        real(wp), parameter :: stpmx = 100e0
        integer :: i, j, k, its, n
        real(wp), intent(in), optional :: parin(:)
        real(wp) :: f, fold, stpmax
        real(wp), dimension(size(x)) :: c, d, fvcold, g, p, s, t, w, xold
        real(wp), dimension(size(x),size(x)) :: qt, r
        logical :: restrt, sing, check
        real(wp) :: fvec(size(x,1))
        ...
        f = fmin(x, funcv,parin)
     
        ! check if root has been found
        if (maxval(abs(fvec(:))) < 0.01e0*tolf) then
            if(present(check_return))check_return = .false.
            return
        endif
     
        stpmax = stpmx*max(sqrt(dot_product(x(:),x(:))),real(n))
        restrt = .true.
        ...
        do its=1,itermax_root
            ...
            if (restrt) then
                ...
                call fdjac(x, fvec, r, funcv,parin)
            ...
            ! else do Broydn update step
                ...
                where(abs(w(:)) < EPS*(abs(fvec(:))+abs(fvcold(:)))) w(:) = 0e0
     
                ! update for non-noisy components of w
                if(any(abs(w(:)) >= 1e-7))then
            ...
            call lnsrch(xold, fold, g, p, x, f, stpmax, check, funcv,parin)
     
            ! check whether root was found
            if(maxval(abs(fvec(:))) < tolf)then
                if(present(check_return))check_return = .false.
                ! print*, 'line 7017 toolbox', maxval(abs(fvec(:))) 
                return
            endif
    ...
    contains
        ...
        !##########################################################################
        subroutine fdjac(x, fvec, df, funcv,parin)
        ...
        !##########################################################################
        subroutine lnsrch(xold, fold, g, p, x, f, stpmax, check, funcv,parin)
            ...
            real(wp), parameter :: alf = 1.0e-4
            real(wp), parameter :: tolx = epsilon(x)
            integer :: ndum
            real(wp) :: a, alam, alam2=0e0, alamin, b, disc, f2=0e0, pabs, rhs1, rhs2, &
                slope, tmplam
            ...
            alamin = tolx/maxval(abs(p(:))/max(abs(xold(:)),1e0))
            alam = 1e0
            ...
            do
                ...
                ! else do backtracking
                    ...
                    if(abs(alam -1e0) <= 1e-7)then
                        tmplam = -slope/(2e0*(f-fold-slope))
                    else
                        ...
                        if(abs(a) <= 1e-7)then
                            ...
                            disc = b*b-3e0*a*slope
                            if(disc < 0e0)then
                                tmplam = 0.5e0*alam
                            elseif(b <= 0e0)then
                                tmplam = (-b+sqrt(disc))/(3e0*a)
                            else
                                tmplam = -slope/(b+sqrt(disc))
                            endif
                        endif
                        if(tmplam > 0.5e0*alam)tmplam = 0.5e0*alam
                    endif
                endif
                alam2 = alam
                f2 = f
                alam = max(tmplam,0.1e0*alam)
            enddo
        ...
        function fmin(x, funcv,parin)
            ...
            ! interface for the function
            interface
                function funcv(x,parin)
                    use my_kinds_mod
                    implicit none
                    real(wp), intent(in) :: x(:)
                    real(wp), intent(in) :: parin(:)
                    real(wp) :: funcv(size(x, 1))
                end function funcv
            end interface
            ...
            fmin = 0.5e0*dot_product(fvec, fvec)
        ...
        subroutine qrupdt(r,qt,u,v)
            ...
            k = n+1-ifirstloc(abs(u(n:1:-1)) >= 1e-7)
            if(k < 1)k=1
            do i = k-1, 1, -1
        ...
        subroutine rotate(r, qt, i, a, b)
            ...
            real(wp), intent(in) :: a, b
            integer :: n
            real(wp) :: c, fact, s, temp(size(r,1))
            ...
            elseif(abs(a) > abs(b))then
                fact = b/a
                c = sign(1e0/sqrt(1e0+fact**2), a)
                s = fact*c
            else
                fact = a/b
                s = sign(1e0/sqrt(1e0+fact**2), b)
                c=fact*s
            endif
            temp(i:n) = r(i, i:n)
        ...
        function pythag(a, b)
            ...
            absb = abs(b)
            if(absa > absb)then
                pythag = absa*sqrt(1e0+(absb/absa)**2)
            else
                if(abs(absb) <= 1e-7)then
                    pythag = 0e0
                else
                    pythag = absb*sqrt(1e0+(absa/absb)**2)
                endif
            endif
...
!##############################################################################
    ...
    subroutine spline_interp1(yi, c)
        ...
        r(1) = (2e0*yi(0)-5e0*yi(1)+4e0*yi(2)-yi(3))/6e0
        r(n+3) = (2e0*yi(n)-5e0*yi(n-1)+4e0*yi(n-2)-yi(n-3))/6e0
    ...
    subroutine spline_interp2(yi, c)
        ...
        ! interpolation data
        real(wp), intent(in) :: yi(0:, 0:)
     
        ! coefficients for spline interpolation
        real(wp), intent(out) :: c(1:, 1:)
     
     
        !##### OTHER VARIABLES ####################################################
     
        integer :: n(2), j
        real(wp):: tempc(size(c,1),0:(size(yi,2)-1))
    ...
    subroutine spline_interp3(yi, c)
        ...
        ! interpolation data
        real(wp), intent(in) :: yi(0:, 0:, 0:)
     
        ! coefficients for spline interpolation
        real(wp), intent(out) :: c(1:, 1:, 1:)
     
     
        !##### OTHER VARIABLES ####################################################
     
        integer :: n(3), j, j2
        real(wp) :: tempc(size(c,1), size(c,2),0:(size(yi,3)-1))
    ...
    subroutine spline_interp4(yi, c)
        ...
        ! interpolation data
        real(wp), intent(in) :: yi(0:, 0:, 0:, 0:)
     
        ! coefficients for spline interpolation
        real(wp), intent(out) :: c(1:, 1:, 1:, 1:)
     
     
        !##### OTHER VARIABLES ####################################################
     
        integer :: n(4), j, j2, j3
        real(wp) :: tempc(size(c,1), size(c,2), size(c,3), 0:(size(yi,4)-1))
    ...
    subroutine spline_interp5(yi, c)
        ...
        ! interpolation data
        real(wp), intent(in) :: yi(0:, 0:, 0:, 0:, 0:)
     
        ! coefficients for spline interpolation
        real(wp), intent(out) :: c(1:, 1:, 1:, 1:, 1:)
     
     
        !##### OTHER VARIABLES ####################################################
     
        integer :: n(5), j, j2, j3, j4
        real(wp) :: tempc(size(c,1),size(c,2),size(c,3),size(c,3), 0:(size(yi,5)-1))
    ...
    subroutine spline_interp6(yi, c)
        ...
        ! interpolation data
        real(wp), intent(in) :: yi(0:, 0:, 0:, 0:, 0:, 0:)
     
        ! coefficients for spline interpolation
        real(wp), intent(out) :: c(1:, 1:, 1:, 1:, 1:, 1:)
     
     
        !##### OTHER VARIABLES ####################################################
     
        integer :: n(6), j, j2, j3, j4, j5
        real(wp) :: tempc(size(c,1),size(c,2), size(c,3), size(c,4), size(c,5),0:(size(yi,6)-1))
    ...
    subroutine spline_interp7(yi, c)
        ...
        ! interpolation data
        real(wp), intent(in) :: yi(0:, 0:, 0:, 0:, 0:, 0:, 0:)
     
        ! coefficients for spline interpolation
        real(wp), intent(out) :: c(1:, 1:, 1:, 1:, 1:, 1:, 1:)
     
     
        !##### OTHER VARIABLES ####################################################
     
        integer :: n(7), j, j2, j3, j4, j5, j6
        real(wp) :: tempc(size(c,1),size(c,2), size(c,3), size(c,4), size(c,5), size(c,6), 0:(size(yi,7)-1))
    ...
    function spline1(x, c)
        ...
        do j1 = p1, q1
            ...
            if(xtemp1 <= 1e0)then
                phi1 = 4e0+xtemp1**2*(3e0*xtemp1-6e0)
            elseif(xtemp1 <= 2e0)then
                phi1 = (2e0-xtemp1)**3
            else
                phi1 = 0e0
            endif
    ...
    function spline1_grid(x, c, left, right, growth)
        ...
        ! growth rate of grid
        real(wp), intent(in), optional :: growth
     
        ! value of spline function
        real(wp) :: spline1_grid
    ...
    function spline1_complete(x, yi, left, right, growth)
        ...
        ! value of spline function
        real(wp) :: spline1_complete
     
     
        !##### OTHER VARIABLES ####################################################
     
        real(wp) :: spline_temp(1)
    ...
    function spline1_complete_m(x, yi, left, right, growth)
        ...
        ! value of spline function
        real(wp) :: spline1_complete_m(size(x, 1))
     
     
        !##### OTHER VARIABLES ####################################################
     
        real(wp) :: c(1:size(yi, 1)+2)
        real(wp) :: xtemp(1:size(x, 1))
        integer :: n, m, j
    ...
    function spline2(x, c)
        ...
        integer :: n(2), p(2), q(2)
        integer :: j1, j2
        real(wp) :: phi1, xtemp1, phi2, xtemp2
        real(wp) :: s2
        ...
        do j1 = p(1), q(1)
            ...
            do j2 = p(2), q(2)
                ...
                ! calculate basis function
                if(xtemp2 <= 1e0)then
                    phi2 = 4e0+xtemp2**2*(3e0*xtemp2-6e0)
                elseif(xtemp2 <= 2e0)then
                    phi2 = (2e0-xtemp2)**3
                else
                    phi2 = 0e0
                endif
    ...
    function spline2_grid(x, c, left, right, growth)
        ...
        ! left interval endpoint
        real(wp), intent(in) :: left(2)
     
        ! right interval endpoint
        real(wp), intent(in) :: right(2)
     
        ! growth rate of grid
        real(wp), intent(in), optional :: growth(2)
     
        ! value of spline function
        real(wp) :: spline2_grid
    ...
    function spline2_complete(x, yi, left, right, growth)
        ...
        ! value where to evaluate spline
        real(wp), intent(in) :: x(dim)
     
        ! data for spline interpolation
        real(wp), intent(in) :: yi(0:, 0:)
     
        ! left interval endpoint
        real(wp), intent(in) :: left(dim)
     
        ! right interval endpoint
        real(wp), intent(in) :: right(dim)
     
        ! growth rate of grid
        real(wp), intent(in), optional :: growth(dim)
     
        ! value of spline function
        real(wp) :: spline2_complete
    ...
    function spline2_complete_m(x, yi, left, right, growth)
        ...
        ! value of spline function
        real(wp) :: spline2_complete_m(size(x, 1))
     
     
        !##### OTHER VARIABLES ####################################################
     
        real(wp) :: c(1:size(yi, 1)+2, 1:size(yi, 2)+2)
        real(wp) :: xtemp(1:size(x, 1), dim)
        integer :: n, m, j, k
    ...
    function spline3(x, c)
        ...
        integer :: n(3), p(3), q(3)
        integer :: j1, j2, j3
        real(wp) :: phi1, xtemp1, phi2, xtemp2, phi3, xtemp3
        real(wp) :: s2, s3
        !!real(wp) :: phi2_vec(NK2 + 3), phi3_vec(NK3 + 3) 
        real(wp) :: phi2_vec(10), phi3_vec(10) ! NK2 + 3 = 9, NK3 + 3 = 9
        ...
        do j3 = p(3), q(3)
            ...
            ! calculate basis function
            if(xtemp3 <= 1e0)then
                phi3 = 4e0 + xtemp3**2 * (3e0 * xtemp3 - 6e0)
            elseif(xtemp3 <= 2e0)then
                phi3 = (2e0-xtemp3)**3
            else
                phi3 = 0e0
            endif        
            phi3_vec(j3 - p(3) + 1) = phi3
        enddo
        ...
        do j1 = p(1), q(1)
            ...
            !!s2 = 0e0
            !!do j2 = p(2), q(2)
                !#### calculate spline for third dimension #######################
            !!    s3 = 0e0
            !!    do j3 = p(3), q(3)
                    ! calculate spline value
            !!        s3 = s3 + c(j1, j2, j3) * phi3_vec(j3 - p(3) + 1)
            !!    enddo
     
                ! calculate spline value
            !!    s2 = s2 + s3 * phi2_vec(j2 - p(2) + 1)
            !!enddo
            s2 = dot_product(phi2_vec(1 : q(2) - p(2) + 1), matmul(c(j1, p(2):q(2), p(3):q(3)), phi3_vec(1 : q(3) - p(3) + 1)))
    ...
    function spline3_grid(x, c, left, right, growth)
        ...
        ! growth rate of grid
        real(wp), intent(in), optional :: growth(3)
     
        ! value of spline function
        real(wp) :: spline3_grid
    ...
    function spline3_complete_m(x, yi, left, right, growth)
        ...
        ! value of spline function
        real(wp) :: spline3_complete_m(size(x, 1))
     
     
        !##### OTHER VARIABLES ####################################################
     
        real(wp) :: c(1:size(yi, 1)+2, 1:size(yi, 2)+2, 1:size(yi, 3)+2)
        real(wp) :: xtemp(1:size(x, 1), dim)
        integer :: n, m, j, k
    ...
    function spline4(x, c)
        ...
        do j = p, q
            ...
            ! calculate basis function
            if(xtemp <= 1e0)then
                phi = 4e0+xtemp**2*(3e0*xtemp-6e0)
            elseif(xtemp <= 2e0)then
                phi = (2e0-xtemp)**3
            else
                phi = 0e0
            endif
    ...
    function spline4_grid(x, c, left, right, growth)
        ...
        ! left interval endpoint
        real(wp), intent(in) :: left(4)
     
        ! right interval endpoint
        real(wp), intent(in) :: right(4)
     
        ! growth rate of grid
        real(wp), intent(in), optional :: growth(4)
     
        ! value of spline function
        real(wp) :: spline4_grid
    ...
    function spline4_complete_m(x, yi, left, right, growth)
        ...
        ! value of spline function
        real(wp) :: spline4_complete_m(size(x, 1))
    ...
    function spline5(x, c)
        ...
        ! value where to evaluate spline
        real(wp), intent(in) :: x(5)
     
        ! coefficients for spline interpolation
        real(wp), intent(in) :: c(1:, 1:, 1:, 1:, 1:)
    ...
    function spline5_grid(x, c, left, right, growth)
        ...
        ! left interval endpoint
        real(wp), intent(in) :: left(5)
     
        ! right interval endpoint
        real(wp), intent(in) :: right(5)
     
        ! growth rate of grid
        real(wp), intent(in), optional :: growth(5)
     
        ! value of spline function
        real(wp) :: spline5_grid
    ...
    function spline5_complete_m(x, yi, left, right, growth)
        ...
        ! value of spline function
        real(wp) :: spline5_complete_m(size(x, 1))
    ...
    function spline6(x, c)
        ...
        ! value where to evaluate spline
        real(wp), intent(in) :: x(6)
     
        ! coefficients for spline interpolation
        real(wp), intent(in) :: c(1:, 1:, 1:, 1:, 1:, 1:)
    ...
    function spline6_grid(x, c, left, right, growth)
        ...
        ! left interval endpoint
        real(wp), intent(in) :: left(6)
     
        ! right interval endpoint
        real(wp), intent(in) :: right(6)
     
        ! growth rate of grid
        real(wp), intent(in), optional :: growth(6)
     
        ! value of spline function
        real(wp) :: spline6_grid
    ...
    function spline6_complete_m(x, yi, left, right, growth)
        ...
        ! value of spline function
        real(wp) :: spline6_complete_m(size(x, 1))
    ...
    function spline7(x, c)
        ...
        ! value where to evaluate spline
        real(wp), intent(in) :: x(7)
     
        ! coefficients for spline interpolation
        real(wp), intent(in) :: c(1:, 1:, 1:, 1:, 1:, 1:, 1:)
    ...
    function spline7_grid(x, c, left, right, growth)
        ...
        ! left interval endpoint
        real(wp), intent(in) :: left(7)
     
        ! right interval endpoint
        real(wp), intent(in) :: right(7)
     
        ! growth rate of grid
        real(wp), intent(in), optional :: growth(7)
     
        ! value of spline function
        real(wp) :: spline7_grid
    ...
    function spline7_complete_m(x, yi, left, right, growth)
        ...
        ! value of spline function
        real(wp) :: spline7_complete_m(size(x, 1))
...
!##############################################################################
    ...
    subroutine legendre(x1, x2, x, w)
        ...
        real(wp), parameter :: eps = 3.0e-14
        integer, parameter :: maxits = 10
        real(wp), parameter :: pi = 3.14159265358979e0
        integer :: its, j, m, n
        real(wp) :: xl, xm
        real(wp), dimension((size(x)+1)/2) :: p1, p2, p3, pp, z, z1
        ...
        z = cos(pi*(arth(1,1,m)-0.25e0)/(n+0.5e0))
...
!##############################################################################
    ...
    subroutine discretize_AR(rho, mu, sigma_eps, z, pi, w)
       ...
       real(wp), intent(in) :: sigma_eps
    
       ! discrete shock values
       real(wp), intent(out) :: z(:)
    
       ! transition matrix
       real(wp), intent(out) :: pi(:, :)
    
       ! the stationary distribution
       real(wp), intent(out), optional :: w(:)
    
    
       !##### OTHER VARIABLES ####################################################
    
       integer :: n, in
       real(wp) :: psi, sigma_eta
       ...
       sigma_eta = sigma_eps/(1e0-rho**2)
    
       ! determine the transition matrix
       call rouwenhorst_matrix(rho, pi)
    
       ! determine the nodes
       psi = sqrt(real(n-1))*sqrt(sigma_eta)
       do in = 1, n
           z(in) = -psi + 2e0*psi*real(in-1)/real(n-1)
       enddo
       z = z + mu
       ...
       recursive subroutine rouwenhorst_matrix(rho, pi_new)
           ...
           implicit none
           real(wp), intent(in) :: rho
           real(wp), intent(out) :: pi_new(:, :)
           integer :: n
           real(wp) :: p, pi_old(size(pi_new,1)-1, size(pi_new,1)-1)
           ...
           if(n == 2)then
               ...
               pi_new(1:n-1, 2:n  ) = pi_new(1:n-1, 2:n  ) + (1e0-p)*pi_old
               pi_new(2:n  , 1:n-1) = pi_new(2:n  , 1:n-1) + (1e0-p)*pi_old
               pi_new(2:n  , 2:n  ) = pi_new(2:n  , 2:n  ) + p*pi_old
    
               pi_new(2:n-1, :) = pi_new(2:n-1, :)/2e0
           endif
       end subroutine
...
!#######################################
! MODIFIED ROWENHORST FOR NON-GAUSSIAN SHOCKS
!######################################
    subroutine discretize_AR_NG(rho, mu, sigma_eps,zin, z, pi, w)
        ...
        ! discrete shock values
        real(wp), intent(out) :: z(:)
        ! proposed discrete shock values
        real(wp),intent(in), optional :: zin(:)
     ...
     end subroutine discretize_AR_NG
    ...
    subroutine discretize_log_AR(rho, mu, sigma_eps, z, pi, w)
        ...
        real(wp) :: sigma_eta, mu_c, sigma_c
        ...
        sigma_c = log(1e0+sigma_eta/mu**2)
        mu_c  = log(mu)-0.5e0*sigma_c
        sigma_c = sigma_c*(1e0-rho**2)
...
!############################################################################## 
    ...
    subroutine execplot(xlim, xticks, xtrange, xlabel, ylim, yticks, ytrange, ylabel, title, &
        ...
        ! x axis definitiion
        real(wp), optional :: xlim(2)

        ! x axis tick definitions
        real(wp), optional :: xticks
        
        ! x axis range for ticks
        real(wp), optional :: xtrange(2)

        ! y axis definition
        real(wp), optional :: ylim(2)

        ! y axis tick definitions
        real(wp), optional :: yticks
        
        ! y axis range for ticks
        real(wp), optional :: ytrange(2)
        ...
        INQUIRE(file='/usr/local/include/toolbox_visual.txt',Exist=exist)
        ...
        if(nw_copy)then
            ...
            if(present(ratio) .and. ft /= "png")then
                write(term, '(a,f4.2)')trim(term)//" size 5,", 5e0*ratio
            endif
        ...
        if(gnu_histogram)then
            if(present(ylim))then
                write(213659, '(a,e13.5,a,e13.5,a)') &
                    'set yrange [',min(minval(ylim),0e0),':',maxval(ylim),']'
            else
        ...
        call system('gnuplot -p "'//trim(cfile)//'"') 
    ...
    subroutine plot(xin, yin, color, linewidth, dashtype, marker, markersize, noline, legend)
        ...
        if(lines)then
            if(present(linewidth)) then
                write(gnu_definitions(gnu_mmax), '(a,f8.2)')trim(gnu_definitions(gnu_mmax))//' lw ', max(linewidth, 0e0)
            else
                write(gnu_definitions(gnu_mmax), '(a,f8.2)')trim(gnu_definitions(gnu_mmax))//' lw ', 2e0
            endif
        ...
        if(points)then
            ...
            if(present(markersize))then
                write(gnu_definitions(gnu_mmax), '(a,f8.2)')trim(gnu_definitions(gnu_mmax))//' ps ', max(markersize, 0e0)
            else
                write(gnu_definitions(gnu_mmax), '(a,f8.2)')trim(gnu_definitions(gnu_mmax))//' ps ', 1e0
            endif
        endif               
    ...
    subroutine plot_hist_new(xvalues, nbins, left, right, incl_outside, absolute, color, legend)
        ...
        ! input variables
        real(wp), intent(in) :: xvalues(1:)

        ! number of bins
        integer, intent(in) :: nbins

        ! left end of the bins
        real(wp), optional :: left

        ! right end of the bins
        real(wp), optional :: right
        ...
        integer :: n, i1
        real(wp) :: lower, upper, total
        real(wp) :: thresholds(0:nbins), freq(0:nbins)
        real(wp) :: xin(nbins), yin(nbins)
        ...
        ! get frequency
        freq(0) = real(count(xvalues < lower))
        do i1 = 1, nbins
            freq(i1) = real(count(xvalues <= thresholds(i1)))
        enddo
        ...
        if(present(incl_outside))then
            if(incl_outside)then
                freq(0) = 0e0
                freq(nbins) = freq(nbins) + real(count(xvalues >= upper))
                total = freq(nbins)
            endif
        endif
        ...
        ! get histogram data
        do i1 = 1, nbins
            xin(i1) = (thresholds(i1-1) + thresholds(i1))/2e0
    ...
    subroutine plot3d_grid(xin, yin, zin, color, linewidth, dashtype, marker, markersize, noline, &
        ...
        real(wp), optional :: ytrange(2)
           ...
           ! z axis definition
        real(wp), optional :: zlim(2)

        ! z axis tick definitions
        real(wp), optional :: zticks

        ! x axis range for ticks
        real(wp), optional :: ztrange(2)

        ! point where the z axis is placed
        real(wp), optional :: zlevel

        ! rotates the graph
        real(wp), optional :: view(2)
        ...
        logical :: lines, points, nw_copy,exist
        ...
        if(lines)then
            if(present(linewidth)) then
                write(definition, '(a,f8.2)')trim(definition)//' lw ', max(linewidth, 0e0)
            else
                if(present(surf))then
                    write(definition, '(a,f8.2)')trim(definition)//' lw ', 0.3e0
                else
                    write(definition, '(a,f8.2)')trim(definition)//' lw ', 1e0
                endif
            endif
        ...
        if(points)then
            ...
            if(present(markersize))then
                write(definition, '(a,f8.2)')trim(definition)//' ps ', max(markersize, 0e0)
            else
                write(definition, '(a,f8.2)')trim(definition)//' ps ', 1e0
            endif
        endif               
        ...
        if(present(zlevel)) then
            write(213659, '(a,e13.5)')'set ticslevel ', -min(max(zlevel, 0e0), 1e0)
        else 
        ...
        if(present(view)) then
            write(213659, '(a,e13.5,a,e13.5)')'set view ', min(max(view(1), 0e0), 360e0), &
                ',',min(max(view(2), 0e0), 360e0)
        endif
...
!############################################################################## 
    ...
    ! SUBROUTINE sort_r
    !
    ! Sorts an array of type real(wp) in ascending order.
    !
    ! PARTS OF THIS PROCEDURE WERE COPIED AND ADAPTED FROM:
    ...
    ! SUBROUTINE sort_r2
    !
    ! Sorts an array of type real(wp) in ascending order and returns new ordering.
    !
    ! PARTS OF THIS PROCEDURE WERE COPIED AND ADAPTED FROM:
    !     The Wikibook "Algorithm Implementation" available at
    !     https://en.wikibooks.org/wiki/Algorithm_Implementation/Sorting/Quicksort#FORTRAN_90.2F95
	...
	!! The following two subroutines were added for faster computation
	
    !##############################################################################
    ! Subroutine spline3_2d
    !
    ! Subroutine for evaluation of three-dimensional spline (array version)
    !##############################################################################
    subroutine spline3_2d(x, c, spline_array)
        ...
        ! value of spline function
        real(wp), intent(out) :: spline_array(:,:)
		
     
        !##### OTHER VARIABLES ####################################################
     
        integer :: n(3), p(3), q(3)
        integer :: j1, j2, j3, i, j, ns
        real(wp) :: phi1, xtemp1, phi2, xtemp2, phi3, xtemp3
        real(wp) :: s2, s3
        !! real(wp) :: phi1_vec(NK1 + 3), phi2_vec(NK2 + 3), phi3_vec(NK3 + 3)
        real(wp) :: phi1_vec(20), phi2_vec(10), phi3_vec(10) ! NK1 + 3 = 18, NK2 + 3 = 9, NK3 + 3 = 9
        ...
        spline_array = 0e0
	    ns = size(spline_array, 1)
		
	    do i = 1, ns
		    do j = 1, ns
                do j1 = p(1), q(1)
                    ! calculate value where to evaluate basis function
                    s2 = dot_product(phi2_vec(1 : (q(2) - p(2) + 1)), matmul(c(j1, p(2):q(2), p(3):q(3), i, j), phi3_vec(1 : (q(3) - p(3) + 1))))
     
                    ! calculate spline value
                    spline_array(i, j) = spline_array(i, j) + s2 * phi1_vec(j1 - p(1) + 1)
                enddo
		    enddo
		enddo
    
    end subroutine spline3_2d
	
	!##############################################################################
    ! SUBROUTINE spline3_grid_2d (array version)
    !
    ! Subroutine for evaluation of three-dimensional spline, includes inverting grid.
    !##############################################################################
    subroutine spline3_grid_2d(x, c, left, right, spline_array, growth)
        !##### INPUT/OUTPUT VARIABLES #############################################
        ...
        ! calculate array of spline values
		call spline3_2d(xtemp, c, spline_array)
    
    end subroutine spline3_grid_2d
 ...
 !##############################################################################
    ...
    ! Subroutine for evaluation of three-dimensional spline (array version)
    !##############################################################################
    subroutine spline2_2d(x, c, spline_array)
        ...
        integer :: n(2), p(2), q(2)
        integer :: j1, j2, i, j, ns
        real(wp) :: phi1, xtemp1, phi2, xtemp2
        real(wp) :: s2
        real(wp) :: phi1_vec(20), phi2_vec(10) ! NK1 + 3 = 18, NK2 + 3 = 9
        ...
        ! Continue with the loop calculation of the spline value
        do i = 1, ns
            do j = 1, ns
                do j1 = p(1), q(1)
                    ! calculate value where to evaluate basis function
                    s2 = dot_product(phi2_vec(1 : q(2) - p(2) + 1), c(j1, p(2):q(2), i, j))
    ...
    end subroutine spline2_2d  
      ! SUBROUTINE spline3_grid_2d (array version)
    !
    ! Subroutine for evaluation of three-dimensional spline, includes inverting grid.
    !##############################################################################
    subroutine spline2_grid_2d(x, c, left, right, spline_array, growth)
        !##### INPUT/OUTPUT VARIABLES #############################################
        ...
        do j = 1, size(x, 1)
            ...
            ! invert grid
            if(present(growth))then
                xtemp(j) = grid_Inv_Grow(x(j), left(j), right(j), growth(j), n_spl)
            else
                xtemp(j) = grid_Inv_Equi(x(j), left(j), right(j), n_spl)
            endif
        enddo
     
        ! calculate array of spline values
		call spline2_2d(xtemp, c, spline_array)
    
    end subroutine spline2_grid_2d
