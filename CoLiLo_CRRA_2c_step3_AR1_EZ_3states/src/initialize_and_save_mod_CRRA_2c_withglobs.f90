...
contains
   ...
   subroutine initialvalues_CRRA(dir,cntry, def_cntry)
      ...
      integer :: ik1, ik2,ik3,ik4, is1, is2
      ...
      if (doinitval) then
               ...
               do ik3 =0, NK3
                  ...
                  do is1 = 1, NS
                     do is2 = 1, NS
                        ...
                        pfdummy(1) = pf_sols(ik1,ik2,ik3,ik4, is1, is2)
                        kappa = kappaS(ik1)
                        lb = price_down
                        ub = price_up
                        parin0(1) = real(is1, 8)
                        ...
                        ! future X state
                        pf_sols(ik1,ik2,ik3,ik4, is1, is2) = pf

                        ! note that preferences are V=(1-beta)*U(c,l)+beta*E(V)
                        v1(ik1,ik2,ik3,ik4, is1, is2) = -(C1**(1e0_wp - 1e0_wp/gamma)/(1e0_wp - 1e0_wp/gamma) - &
                           chhi*L1**(1.0e0_wp + varphi)/(1.0e0_wp + varphi))  ! check sidn Rud&Swans
                        v2(ik1,ik2,ik3,ik4, is1, is2) = -(C2**(1e0_wp - 1e0_wp/gamma)/(1e0_wp - 1e0_wp/gamma) - &
                           chhi*L2**(1.0e0_wp + varphi)/(1.0e0_wp + varphi))

                        s1(ik1,ik2,ik3,ik4, is1, is2) = (C1**(-1e0_wp/gamma)*(C1 - ph*Y1))/(1.0_wp - beta)
         ...
         pf_sols_aut = pf_sols(pos_zeroK,:,:,pos_zeroOm, :, :)
         v1_aut = v1(pos_zeroK,:,:,pos_zeroOm, :, :);
         v2_aut = v2(pos_zeroK,:,:,pos_zeroOm, :, :);
   ...
   subroutine save_results_CRRA(dir,cnt_mom)
      ...
      integer :: ik1, unitnumber1, unitnumber2, unitnumber3, nlines,status
      logical :: dir_exists
      real(kind=wp), dimension(:,:), allocatable :: tmp1, tmp2
      ! 101 format(1x, *(g0, ", "))
      ...
      allocate(tmp1(size(matrix_csv_v1,1),size(matrix_csv_v1,2)))
      tmp1 = RESHAPE(source=v1, &
         shape=shape(matrix_csv_v1))
         matrix_csv_v1=tmp1 
      tmp1 = RESHAPE(source=v2, &
         shape=shape(matrix_csv_v2))
      matrix_csv_v2=tmp1
         tmp1 = RESHAPE(source=s1, &
         shape=shape(matrix_csv_s1))
      matrix_csv_s1=tmp1
      
      tmp1 = RESHAPE(source=pf_sols, &
      ...
      ! save coefficients
      tmp1 = RESHAPE(source=coeff_v1, &
         shape=shape(matrix_coefs_csv_v1))
         matrix_coefs_csv_v1=tmp1
         tmp1 = RESHAPE(source=coeff_v2, &
         shape=shape(matrix_coefs_csv_v2))
         matrix_coefs_csv_v2=tmp1
      tmp1 = RESHAPE(source=coeff_s1, &
         shape=shape(matrix_coefs_csv_s1))
         matrix_coefs_csv_s1=tmp1
...
deallocate(tmp1)
      ! SAVE AUTARKY
allocate(tmp2(size(matrix_csv_aut_v1,1),size(matrix_csv_aut_v1,2)))


      tmp2 = RESHAPE(source=v1_aut, &
         shape=shape(matrix_csv_aut_v1))
         matrix_csv_aut_v1=tmp2
      tmp2 = RESHAPE(source=v2_aut, &
         shape=shape(matrix_csv_aut_v2))
         matrix_csv_aut_v2=tmp2
      tmp2 = RESHAPE(source=pf_sols_aut, &
         shape=shape(matrix_csv_aut_pf))
         matrix_csv_aut_pf=tmp2
