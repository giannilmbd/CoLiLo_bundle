subroutine fit_ls(n,m0,x,y,beta,std_err,std_err_res,t_test)
...
real(kind=wp) :: std_err(m0+1),std_err_res,t_test(m0+1)
...
residual=y-matmul(z,beta)
print*,'sum residuals',sum(residual,1)
std_err_res=sqrt(sum((residual)**2/(n-m),1))
do cnt=1,m
std_err(cnt)=std_err_res/sqrt(zpz_inv(cnt,cnt))
t_test(cnt)=beta(cnt)/std_err(cnt)
enddo
