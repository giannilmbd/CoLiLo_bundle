...
module minpack_module
    ...
    abstract interface
        subroutine func(n, x, fvec, iflag,data_)
            ...
            real(wp),intent(in), dimension(:),optional :: data_
        end subroutine func
...
!  approximating the nonzero terms.
    ...
    subroutine fdjac1(fcn, n, x, Fvec, Fjac, Ldfjac, Iflag, Ml, Mu, Epsfcn, Wa1, Wa2,data_)
        ...
        real(wp), intent(in) :: Epsfcn !! an input variable used in determining a suitable
                                  ...
                                  !! precision.
        real(wp), intent(in),dimension(:),optional :: data_       !! Added by me: data to be passed to the function
    ...
    subroutine hybrd(fcn, n, x, Fvec, Xtol, Maxfev, Ml, Mu, Epsfcn, Diag, Mode, &
        ...
        main : block
            ...
            outer : do
                ...
                iflag = 2
                call fdjac1(fcn, n, x, Fvec, Fjac, Ldfjac, iflag, Ml, Mu, Epsfcn, Wa1, Wa2,data_)
                Nfev = Nfev + msum
                if (iflag < 0) exit main
                ...
                inner : do
                    ...
                    if (Nprint > 0) then
                        iflag = 0
                        if (mod(iter - 1, Nprint) == 0) call fcn(n, x, Fvec, iflag,data_)
                        if (iflag < 0) exit main
                    end if
                    ...
                    iflag = 1
                    call fcn(n, Wa2, Wa4, iflag,data_)
                    Nfev = Nfev + 1
                    if (iflag < 0) exit main
        ...
        iflag = 0
        if (Nprint > 0) call fcn(n, x, Fvec, iflag,data_)
...
!  approximation.
    ...
    subroutine hybrd1(fcn, n, x, Fvec, Tol, Info, Wa, Lwa,data_)
        ...
        if (n > 0 .and. Tol >= zero .and. Lwa >= (n*(3*n + 13))/2) then
            ...
            call hybrd(fcn, n, x, Fvec, xtol, maxfev, ml, mu, epsfcn, Wa(1), mode, &
                       ...
                       Wa(n + 1), Wa(2*n + 1), Wa(3*n + 1), Wa(4*n + 1), Wa(5*n + 1),data_)
                       else 
                       call hybrd(fcn, n, x, Fvec, xtol, maxfev, ml, mu, epsfcn, Wa(1), mode, &
                       factor, nprint, Info, nfev, Wa(index + 1), n, Wa(6*n + 1), lr, &
