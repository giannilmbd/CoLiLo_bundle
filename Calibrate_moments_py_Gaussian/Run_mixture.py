#%%
# This code calibrates a Gaussian Mixture model 
...
# %% ------------------------------------  Load Packages ------------------------------------ #
import os
os.chdir('/home/gianni/Dropbox/projects/ANNAGIANCARLOGIANNI/CoLiLoGlobal/models/CoLiLoCodes4Circulation/Calibrate_moments_py_Gaussian')
import numpy as np
import pandas as pd
...
#%% --------------------------------------------SET PARAMETERS -------------------------------------------------------- #
Nm = 5 #number of points for discretization (9 and 25 is what we considered so far)
#%% --------------------------------------------------------------------------------------------------------------------#
...
p2=.3
p3=1-p1-p2
pC = np.array([1.0])# np.array([p1, p2, p3])
# means
muc1=.10
muc2=0.0
muc3=0.0
...
def country_level(cnt_c,country_id,muC,pC,sigmaC,Nm,emp_moms_csv,n_moments): 
    ...
    print("\033[34m Done country " + str(cnt_c) + "\033[0m")
    # print(cntry+"\n")
    # Load the empirical moments of the AR process
    A1=emp_moms_csv[emp_moms_csv['cc']==cntry]['autocorr_coef'].values[0]
    emp_m1=0.0
    ...
    pC=np.array([1.0])# np.array(pC_optimal)
    # convert results into arrays to be used in the discretization
    muC=np.array([0.0])#np.array(muC_optimal)
    resid=np.array([0.0])
    ...
    p_even1, x_even1,match_order,resid = discrete_gmar(mu, A1, pC, np.array([emp_m1]),
                                                        np.array([np.power(emp_m2, 0.5)]), Nm, n_moments,
                                                        method='even',
                                                        country=cntry)#'GMQ''even''gauss-hermite'

    return p_even1, x_even1,match_order,resid


#%%  --------------------- for debugging purposes ---------------------
cntry="FRA"
i=int(np.where(country_id==cntry)[0])
sigmaC=np.array([emp_moms_csv[emp_moms_csv['cc']==cntry]['stdev'].values[0]])
muC=0.0
P, X,c,d=country_level(i,country_id,muC,pC,sigmaC,Nm,emp_moms_csv,n_moments)
print('residual: ',d)


def simulate_discretized_process(P, X, n_steps=1000, n_simulations=1000):
    """
    Simulate the discretized process to estimate moments.

    Parameters:
        P (ndarray): Transition probability matrix (Nm**K x Nm**K).
        X (ndarray): Discretized states (K x Nm**K).
        n_steps (int): Number of time steps to simulate.
        n_simulations (int): Number of simulation paths.

    Returns:
        simulated_moments (dict): Dictionary containing simulated mean, variance, skewness.
    """
    n_states = P.shape[0]
    ...
    simulated_paths = np.zeros((n_simulations, n_steps, K))

    for sim in range(n_simulations):
        # Initialize the state
        current_state = np.random.choice(state_indices, p=initial_probs)

        for t in range(n_steps):
            # Record the current state values
            simulated_paths[sim, t, :] = X[:, current_state]

            # Transition to the next state
            current_state = np.random.choice(state_indices, p=P[current_state, :])

    # Calculate simulated moments
    simulated_mean = np.mean(simulated_paths, axis=(0, 1))  # Mean across all paths and time steps
    simulated_variance = np.var(simulated_paths, axis=(0, 1))  # Variance
    simulated_skewness = (
        np.mean(((simulated_paths - simulated_mean) ** 3), axis=(0, 1)) /
        (simulated_variance ** 1.5)
    )  # Skewness

    simulated_moments = {
        "mean": simulated_mean,
        "variance": simulated_variance,
        "skewness": simulated_skewness
    }

    return simulated_moments


def compare_moments(simulated_moments, input_moments):
    """
    Compare simulated moments with input moments.

    Parameters:
        simulated_moments (dict): Dictionary containing simulated mean, variance, skewness.
        input_moments (dict): Dictionary containing input mean, variance, skewness.

    Returns:
        errors (dict): Dictionary containing absolute errors for each moment.
    """
    errors = {}
    for moment in simulated_moments:
        errors[moment] = np.abs(simulated_moments[moment] - input_moments[moment])
        print(f"{moment.capitalize()} - Input: {input_moments[moment]}, Simulated: {simulated_moments[moment]}, Error: {errors[moment]}")
...
# Discretization results from discrete_gmar
# P, X, _, _ = discrete_gmar(mu, A1, pC, muC,
#                                                     sigmaC, Nm, n_moments,
#                                                     method='even',
#                                                     country=cntry)
...
if sim==True:
    # Input moments
    input_moments = {
    "mean": np.array(0),  # Replace T1 with the true input mean
    "variance": np.array(sigmaC**2.0),  # Replace with true input variance
    "skewness": np.array(0)  # Replace with true input skewness
    }
    # Simulate the discretized process
    simulated_moments = simulate_discretized_process(P, X)

    # Compare moments
    compare_moments(simulated_moments, input_moments)

#%% FOR DEBUGGING (place a db stop in discreteGMAR, then run this)
num_cases=len(country_id) #4 #for debugging # Give country you want to debug
all_probs=np.empty((num_cases,Nm*Nm)) # initialize matrices of probabilities and discretized shocks for each country
all_shocks=np.empty((num_cases,Nm)) # shocks
all_match=np.empty((num_cases,Nm)) # match order
all_resids=np.empty((num_cases,1)) # residuals
tab_results=pd.DataFrame(data={'cc':country_id,'Residual':None,'AverageMatch':None}) # results
#
# IF RUN SEQUENTIALLY USE THIS BLOCK
results = np.empty((num_cases, 4), dtype=object)
for cnt_c in range(num_cases):
    out1,out2,out3,out4=country_level(cnt_c,country_id,muC,pC,sigmaC,Nm,emp_moms_csv,n_moments)
    results[cnt_c,0]=out1
    results[cnt_c,1]=out2
    results[cnt_c,2]=out3
    results[cnt_c,3]=out4
#%%
# Parallelize process by country
# first contruct the list of arguments to be passed to the function
#%%
# items = [(i,country_id,muC,pC,sigmaC,Nm,emp_moms_csv,n_moments) for i in range(num_cases)]
...
for cnt_c in range(num_cases):
    ...
    print("\033[33m Done country " + str(cnt_c) + "\033[0m")
    print("\033[33m Done country "+country_id[cnt_c]+"\033[0m")
    print(f"\033[33m Country number:\033[0m {cnt_c}")
...
df1.to_csv("Pmatrices_"+str(Nm)+".csv",
           index=False,header=False)
df1=pd.DataFrame(all_shocks)
df1.to_csv("Xmatrices_"+str(Nm)+".csv", 
           index=False,header=False)
# Create a DataFrame for residuals
all_resids2 = all_resids.reshape((-1, 1))
all_resids2 = all_resids2.flatten() 
residuals_df = pd.DataFrame({'Country': country_id[range(num_cases)], 'Residual': all_resids2[range(num_cases)]})

# Write residuals to a CSV file
residuals_df.to_csv("residuals.csv", index=False)
print('DONE')
# RUN ONLY ABOVE IF DON'T WANT TO CHECK RESULTS HERE
# import sys 
# sys.exit()
...
plt.savefig('Approx_distros.png', bbox_inches='tight', pad_inches = 0) #dpi=100,
...
#%%
emp_new=emp_moms_csv.iloc[0:62,0:] 
emp_new['Variance_emp']=emp_new['stdev'].values**2
...
with (ro.default_converter + pandas2ri.converter).context():
  ...
  r_tab_results = ro.conversion.get_conversion().py2rpy(tab_results.iloc[0:62,0:])
...
ggplot2.geom_point(ggplot2.aes_string(x='Variance', y='var_ar'))+\
...
ggplot2.geom_line(ggplot2.aes_string(x='Variance', y='Variance'),color='red',linetype='dashed') +\
 ggrepel.geom_text_repel(ggplot2.aes_string(x='Variance', y='var_ar',label = 'cc'))+\
ggplot2.geom_smooth(ggplot2.aes_string(x='Variance', y='var_ar'),method='lm')
p2.plot()
display(image_png(p2))
robjects.r.ggsave(filename="variance_emp_disc.png", plot=p2, width=200, height=150, unit='mm')
#%% SKEWNESS
p3=ggplot2.ggplot(data=tab_all)+\
ggplot2.geom_point(ggplot2.aes_string(x='Skewness', y='skewness_ar') )+\
...
ggplot2.geom_line(ggplot2.aes_string(x='Skewness', y='Skewness'),color='red',linetype='dashed') +\
 ggrepel.geom_text_repel(ggplot2.aes_string(x='Skewness', y='skewness_ar',label = 'cc'))+\
ggplot2.geom_smooth(ggplot2.aes_string(x='Skewness', y='skewness_ar') ,method='lm')
p3.plot()
display(image_png(p3))
#  ggplot2.geom_text(ggplot2.aes_string(label = 'cc'), hjust = 0, vjust = 0)+ \
robjects.r.ggsave(filename="skewness_emp_disc.png", plot=p3, width=200, height=150, unit='mm')
#%% KURTOSIS
p4=ggplot2.ggplot(tab_all)+\
ggplot2.geom_point(ggplot2.aes_string(x='Kurtosis', y='kurtosis_ar'))+\
...
ggplot2.geom_line(ggplot2.aes_string(x='Kurtosis', y='Kurtosis'),color='red',linetype='dashed') +\
 ggrepel.geom_text_repel(ggplot2.aes_string(x='Kurtosis', y='kurtosis_ar',label = 'cc'))+\
ggplot2.geom_smooth(ggplot2.aes_string(x='Kurtosis', y='kurtosis_ar'),method='lm')
p4.plot()
display(image_png(p4))
robjects.r.ggsave(filename="kurtosis_emp_disc.png", plot=p4, width=200, height=150, unit='mm')
# display(image_png(p))

#%%    
tab_results2=tab_results.iloc[:num_cases,1:]
tab_results2.to_csv('calib_results.csv',index=False,header=True)
tab_results_reset_index = tab_results2.rename_axis('index')
tab_results_reset_index.to_latex("calib_results.tex",index_names=False, 
                     header=True,
