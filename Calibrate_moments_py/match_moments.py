...
def match_moments(empirical_moments, muC_init, pC_init, sigmaC_init,method='an',country='NaC'):
    ...
    def objective(params):
        muC = params[:3]
        pC = np.abs(params[3:5])
        sigmaC = np.abs(params[5:])
        if np.sum(pC)>=1:
            res_pC=1-np.sum(pC)
            pC=pC+res_pC/2.0
        ...
        d2=np.linalg.norm(np.array([var_,Sk,Kur])/empirical_moments[1:]-1.0)
    ...
    initial_params = list(muC_init)+list(pC_init)+list(sigmaC_init)

    # Set the bounds for the optimization variables
    bounds = [(None, None)] * len(muC_init) + [(1E-8, 0.999999)] * len(pC_init) + [(1E-8, None)] * len(sigmaC_init)
    ...
    def sum_constraint(pC):
        return np.sum(pC) - 1.0
    def callback(xk):
